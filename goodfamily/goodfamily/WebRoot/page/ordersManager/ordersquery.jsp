...
<form id="queryForm" action="<%=path %>/oni/ordersQuery.action" method="post">

	<s:if test="%{orders.beginIndex!=null}">
 		<input type="hidden" id="beginIndex" value='<s:property value="orders.beginIndex"/>' name="orders.beginIndex"/>
	</s:if>
 	<s:else>
 		<input type="hidden" id="beginIndex" value='1' name="orders.beginIndex"/>
 	</s:else>
...
<div class="mt10" id="table">
    <div class="box span10 oh">
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="0" class="list_table CRZ" id="CRZ0">
            ...
            <s:iterator value="ordersList" id="orders">
           	<tr class="tr" style="background-color: rgb(255, 255, 255);">
                <td class="td_center"><input type="checkbox" name="selectCheckbox" value='<s:property value="#orders.tid"/>'></td>
                                                                
                <td><s:property value="#orders.type"></s:property></td>
                                                
                <td><s:property value="#orders.state"></s:property></td>
                                                
                <td><s:property value="#orders.fwsId"></s:property></td>
                                                
                <td><s:property value="#orders.kehuId"></s:property></td>
                                                
                <td><s:property value="#orders.mpcenterId"></s:property></td>
                                                
                <td><s:property value="#orders.ksdate"></s:property></td>
                                                
                <td><s:property value="#orders.jsdate"></s:property></td>
                                                
                <td><s:property value="#orders.zj"></s:property></td>
                                                
                <td><s:property value="#orders.zsj"></s:property></td>
...
<div id="w" class="easyui-window" title="表单提交" data-options="modal:true,closed:true,iconCls:'icon-save'"
    ...
    <iframe id="frameframe" src="<%=path%>/page/ordersManager/ordersadd.jsp" style="width: 100%; height: 100%"></iframe>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn_add").bind({
            click: function () {
            	$("#frameframe").attr("src","<%=path%>/page/ordersManager/ordersadd.jsp");
                $('#w').window('open');
            }
        });
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#frameframe").attr("src","<%=path%>/oni/ordersEdit.action?orders.tid="+selectId);
           	                $('#w').window('open');
           		       }
           			});
           		}
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#queryForm").attr("action","<%=path %>/oni/ordersDeleteDone.action?orders.tid="+selectId);
           		        	$("#queryForm").submit();
           		       }
           			});
           		}
