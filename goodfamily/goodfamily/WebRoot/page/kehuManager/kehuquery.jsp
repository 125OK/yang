...
<form id="queryForm" action="<%=path %>/oni/kehuQuery.action" method="post">

	<s:if test="%{kehu.beginIndex!=null}">
 		<input type="hidden" id="beginIndex" value='<s:property value="kehu.beginIndex"/>' name="kehu.beginIndex"/>
	</s:if>
 	<s:else>
 		<input type="hidden" id="beginIndex" value='1' name="kehu.beginIndex"/>
 	</s:else>
...
<div class="mt10" id="table">
    <div class="box span10 oh">
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="0" class="list_table CRZ" id="CRZ0">
            ...
            <s:iterator value="kehuList" id="kehu">
           	<tr class="tr" style="background-color: rgb(255, 255, 255);">
                <td class="td_center"><input type="checkbox" name="selectCheckbox" value='<s:property value="#kehu.tid"/>'></td>
                                                                
                <td><s:property value="#kehu.name"></s:property></td>
                                                
                <td><s:property value="#kehu.phone"></s:property></td>
                                                
                <td><s:property value="#kehu.address"></s:property></td>
                                                
                <td><s:property value="#kehu.yue"></s:property></td>
                                                
                <td><s:property value="#kehu.fx"></s:property></td>
                                                
                <td><s:property value="#kehu.mj"></s:property></td>
                                                
                <td><s:property value="#kehu.state"></s:property></td>
...
<div id="w" class="easyui-window" title="表单提交" data-options="modal:true,closed:true,iconCls:'icon-save'"
    ...
    <iframe id="frameframe" src="<%=path%>/page/kehuManager/kehuadd.jsp" style="width: 100%; height: 100%"></iframe>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn_add").bind({
            click: function () {
            	$("#frameframe").attr("src","<%=path%>/page/kehuManager/kehuadd.jsp");
                $('#w').window('open');
            }
        });
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#frameframe").attr("src","<%=path%>/oni/kehuEdit.action?kehu.tid="+selectId);
           	                $('#w').window('open');
           		       }
           			});
           		}
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#queryForm").attr("action","<%=path %>/oni/kehuDeleteDone.action?kehu.tid="+selectId);
           		        	$("#queryForm").submit();
           		       }
           			});
           		}
