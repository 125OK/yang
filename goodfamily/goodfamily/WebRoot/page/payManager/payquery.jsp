...
<form id="queryForm" action="<%=path %>/oni/payQuery.action" method="post">

	<s:if test="%{pay.beginIndex!=null}">
 		<input type="hidden" id="beginIndex" value='<s:property value="pay.beginIndex"/>' name="pay.beginIndex"/>
	</s:if>
 	<s:else>
 		<input type="hidden" id="beginIndex" value='1' name="pay.beginIndex"/>
 	</s:else>
    ...
    <div class="box span10 oh">
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="0" class="list_table CRZ" id="CRZ0">
            ...
            <tr>
                ...
                <th style="width: 100px;">充值方式</th>
                                                
                <th style="width: 100px;">单号</th>
                                                
                <th style="width: 100px;">充值日期</th>
                                     
            </tr>
            <s:iterator value="payList" id="pay">
           	<tr class="tr" style="background-color: rgb(255, 255, 255);">
                <td class="td_center"><input type="checkbox" name="selectCheckbox" value='<s:property value="#pay.tid"/>'></td>
                                                                
                <td><s:property value="#pay.kehuId"></s:property></td>
                                                
                <td><s:property value="#pay.payMoney"></s:property></td>
                                                
                <td><s:property value="#pay.payType"></s:property></td>
                                                
                <td><s:property value="#pay.payCode"></s:property></td>
                                                
                <td><s:property value="#pay.payDate"></s:property></td>
...
<div id="w" class="easyui-window" title="表单提交" data-options="modal:true,closed:true,iconCls:'icon-save'"
    ...
    <iframe id="frameframe" src="<%=path%>/page/payManager/payadd.jsp" style="width: 100%; height: 100%"></iframe>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn_add").bind({
            click: function () {
            	$("#frameframe").attr("src","<%=path%>/page/payManager/payadd.jsp");
                $('#w').window('open');
            }
        });
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#frameframe").attr("src","<%=path%>/oni/payEdit.action?pay.tid="+selectId);
           	                $('#w').window('open');
           		       }
           			});
           		}
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#queryForm").attr("action","<%=path %>/oni/payDeleteDone.action?pay.tid="+selectId);
           		        	$("#queryForm").submit();
           		       }
           			});
           		}
