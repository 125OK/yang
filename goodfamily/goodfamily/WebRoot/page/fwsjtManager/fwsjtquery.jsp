...
<form id="queryForm" action="<%=path %>/oni/fwsjtQuery.action" method="post">

	<s:if test="%{fwsjt.beginIndex!=null}">
 		<input type="hidden" id="beginIndex" value='<s:property value="fwsjt.beginIndex"/>' name="fwsjt.beginIndex"/>
	</s:if>
 	<s:else>
 		<input type="hidden" id="beginIndex" value='1' name="fwsjt.beginIndex"/>
 	</s:else>
    ...
    <div class="box span10 oh">
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="0" class="list_table CRZ" id="CRZ0">
            ...
            <tr>
                ...
                <th style="width: 100px;">子女情况</th>
                                                
                <th style="width: 100px;">有无老人赡养</th>
                                                
                <th style="width: 100px;">其他</th>
                                                
                <th style="width: 100px;">人事部留言</th>
                                                
                <th style="width: 100px;">备注</th>
                                     
            </tr>
            <s:iterator value="fwsjtList" id="fwsjt">
           	<tr class="tr" style="background-color: rgb(255, 255, 255);">
                <td class="td_center"><input type="checkbox" name="selectCheckbox" value='<s:property value="#fwsjt.tid"/>'></td>
                                                                
                <td><s:property value="#fwsjt.fwsId"></s:property></td>
                                                
                <td><s:property value="#fwsjt.jtcy"></s:property></td>
                                                
                <td><s:property value="#fwsjt.phone"></s:property></td>
                                                
                <td><s:property value="#fwsjt.zn"></s:property></td>
                                                
                <td><s:property value="#fwsjt.ywlr"></s:property></td>
                                                
                <td><s:property value="#fwsjt.qita"></s:property></td>
                                                
                <td><s:property value="#fwsjt.rsb"></s:property></td>
                                                
                <td><s:property value="#fwsjt.remark"></s:property></td>
...
<div id="w" class="easyui-window" title="表单提交" data-options="modal:true,closed:true,iconCls:'icon-save'"
    ...
    <iframe id="frameframe" src="<%=path%>/page/fwsjtManager/fwsjtadd.jsp" style="width: 100%; height: 100%"></iframe>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn_add").bind({
            click: function () {
            	$("#frameframe").attr("src","<%=path%>/page/fwsjtManager/fwsjtadd.jsp");
                $('#w').window('open');
            }
        });
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#frameframe").attr("src","<%=path%>/oni/fwsjtEdit.action?fwsjt.tid="+selectId);
           	                $('#w').window('open');
           		       }
           			});
           		}
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#queryForm").attr("action","<%=path %>/oni/fwsjtDeleteDone.action?fwsjt.tid="+selectId);
           		        	$("#queryForm").submit();
           		       }
           			});
           		}
