...
<form id="queryForm" action="<%=path %>/oni/orderinfoQuery.action" method="post">

	<s:if test="%{orderinfo.beginIndex!=null}">
 		<input type="hidden" id="beginIndex" value='<s:property value="orderinfo.beginIndex"/>' name="orderinfo.beginIndex"/>
	</s:if>
 	<s:else>
 		<input type="hidden" id="beginIndex" value='1' name="orderinfo.beginIndex"/>
 	</s:else>
    ...
    <div class="box span10 oh">
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="0" class="list_table CRZ" id="CRZ0">
            ...
            <tr>
                ...
                <th style="width: 100px;">订单时间段id</th>
                                                
                <th style="width: 100px;">服务项</th>
                                                
                <th style="width: 100px;">服务内容</th>
                                     
            </tr>
            <s:iterator value="orderinfoList" id="orderinfo">
           	<tr class="tr" style="background-color: rgb(255, 255, 255);">
                <td class="td_center"><input type="checkbox" name="selectCheckbox" value='<s:property value="#orderinfo.tid"/>'></td>
                                                                
                <td><s:property value="#orderinfo.sjId"></s:property></td>
                                                
                <td><s:property value="#orderinfo.project"></s:property></td>
                                                
                <td><s:property value="#orderinfo.content"></s:property></td>
...
<div id="w" class="easyui-window" title="表单提交" data-options="modal:true,closed:true,iconCls:'icon-save'"
    ...
    <iframe id="frameframe" src="<%=path%>/page/orderinfoManager/orderinfoadd.jsp" style="width: 100%; height: 100%"></iframe>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn_add").bind({
            click: function () {
            	$("#frameframe").attr("src","<%=path%>/page/orderinfoManager/orderinfoadd.jsp");
                $('#w').window('open');
            }
        });
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#frameframe").attr("src","<%=path%>/oni/orderinfoEdit.action?orderinfo.tid="+selectId);
           	                $('#w').window('open');
           		       }
           			});
           		}
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#queryForm").attr("action","<%=path %>/oni/orderinfoDeleteDone.action?orderinfo.tid="+selectId);
           		        	$("#queryForm").submit();
           		       }
           			});
           		}
