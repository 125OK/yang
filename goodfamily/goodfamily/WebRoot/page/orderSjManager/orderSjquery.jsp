...
<form id="queryForm" action="<%=path %>/oni/orderSjQuery.action" method="post">

	<s:if test="%{orderSj.beginIndex!=null}">
 		<input type="hidden" id="beginIndex" value='<s:property value="orderSj.beginIndex"/>' name="orderSj.beginIndex"/>
	</s:if>
 	<s:else>
 		<input type="hidden" id="beginIndex" value='1' name="orderSj.beginIndex"/>
 	</s:else>
...
<div class="mt10" id="table">
    <div class="box span10 oh">
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="0" class="list_table CRZ" id="CRZ0">
            ...
            <s:iterator value="orderSjList" id="orderSj">
           	<tr class="tr" style="background-color: rgb(255, 255, 255);">
                <td class="td_center"><input type="checkbox" name="selectCheckbox" value='<s:property value="#orderSj.tid"/>'></td>
                                                                
                <td><s:property value="#orderSj.orderbId"></s:property></td>
                                                
                <td><s:property value="#orderSj.zhouId"></s:property></td>
                                                
                <td><s:property value="#orderSj.timebId"></s:property></td>
...
<div id="w" class="easyui-window" title="表单提交" data-options="modal:true,closed:true,iconCls:'icon-save'"
    ...
    <iframe id="frameframe" src="<%=path%>/page/orderSjManager/orderSjadd.jsp" style="width: 100%; height: 100%"></iframe>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn_add").bind({
            click: function () {
            	$("#frameframe").attr("src","<%=path%>/page/orderSjManager/orderSjadd.jsp");
                $('#w').window('open');
            }
        });
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#frameframe").attr("src","<%=path%>/oni/orderSjEdit.action?orderSj.tid="+selectId);
           	                $('#w').window('open');
           		       }
           			});
           		}
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#queryForm").attr("action","<%=path %>/oni/orderSjDeleteDone.action?orderSj.tid="+selectId);
           		        	$("#queryForm").submit();
           		       }
           			});
           		}
