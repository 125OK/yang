...
<form id="queryForm" action="<%=path %>/oni/zhouQuery.action" method="post">

	<s:if test="%{zhou.beginIndex!=null}">
 		<input type="hidden" id="beginIndex" value='<s:property value="zhou.beginIndex"/>' name="zhou.beginIndex"/>
	</s:if>
 	<s:else>
 		<input type="hidden" id="beginIndex" value='1' name="zhou.beginIndex"/>
 	</s:else>
    ...
    <div class="box span10 oh">
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="0" class="list_table CRZ" id="CRZ0">
            ...
            <tr>
                ...
                <th style="width: 100px;">周几</th>
                                     
            </tr>
            <s:iterator value="zhouList" id="zhou">
           	<tr class="tr" style="background-color: rgb(255, 255, 255);">
                <td class="td_center"><input type="checkbox" name="selectCheckbox" value='<s:property value="#zhou.tid"/>'></td>
                                                                
                <td><s:property value="#zhou.zhouji"></s:property></td>
...
<div id="w" class="easyui-window" title="表单提交" data-options="modal:true,closed:true,iconCls:'icon-save'"
    ...
    <iframe id="frameframe" src="<%=path%>/page/zhouManager/zhouadd.jsp" style="width: 100%; height: 100%"></iframe>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn_add").bind({
            click: function () {
            	$("#frameframe").attr("src","<%=path%>/page/zhouManager/zhouadd.jsp");
                $('#w').window('open');
            }
        });
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#frameframe").attr("src","<%=path%>/oni/zhouEdit.action?zhou.tid="+selectId);
           	                $('#w').window('open');
           		       }
           			});
           		}
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#queryForm").attr("action","<%=path %>/oni/zhouDeleteDone.action?zhou.tid="+selectId);
           		        	$("#queryForm").submit();
           		       }
           			});
           		}
