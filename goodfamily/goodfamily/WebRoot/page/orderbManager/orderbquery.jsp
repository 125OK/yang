...
<form id="queryForm" action="<%=path %>/oni/orderbQuery.action" method="post">

	<s:if test="%{orderb.beginIndex!=null}">
 		<input type="hidden" id="beginIndex" value='<s:property value="orderb.beginIndex"/>' name="orderb.beginIndex"/>
	</s:if>
 	<s:else>
 		<input type="hidden" id="beginIndex" value='1' name="orderb.beginIndex"/>
 	</s:else>
    ...
    <div class="box span10 oh">
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="0" class="list_table CRZ" id="CRZ0">
            ...
            <tr>
                ...
                <th style="width: 100px;">订单时间段编码</th>
                                                
                <th style="width: 100px;">服务项编码</th>
                                     
            </tr>
            <s:iterator value="orderbList" id="orderb">
           	<tr class="tr" style="background-color: rgb(255, 255, 255);">
                <td class="td_center"><input type="checkbox" name="selectCheckbox" value='<s:property value="#orderb.tid"/>'></td>
                                                                
                <td><s:property value="#orderb.ordersId"></s:property></td>
                                                
                <td><s:property value="#orderb.orderSjId"></s:property></td>
                                                
                <td><s:property value="#orderb.orderinfoId"></s:property></td>
...
<div id="w" class="easyui-window" title="表单提交" data-options="modal:true,closed:true,iconCls:'icon-save'"
    ...
    <iframe id="frameframe" src="<%=path%>/page/orderbManager/orderbadd.jsp" style="width: 100%; height: 100%"></iframe>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn_add").bind({
            click: function () {
            	$("#frameframe").attr("src","<%=path%>/page/orderbManager/orderbadd.jsp");
                $('#w').window('open');
            }
        });
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#frameframe").attr("src","<%=path%>/oni/orderbEdit.action?orderb.tid="+selectId);
           	                $('#w').window('open');
           		       }
           			});
           		}
           		...
           		}else if(selectCount>1)
           		{
           			...
           			var value=$('input:checkbox[name=selectCheckbox]:checked').each(function(i){
           		       ...
           		       if(0==i)
           		       {
           		        	...
           		        	$("#queryForm").attr("action","<%=path %>/oni/orderbDeleteDone.action?orderb.tid="+selectId);
           		        	$("#queryForm").submit();
           		       }
           			});
           		}
